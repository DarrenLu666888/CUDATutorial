...
int main(){
    ...
    const int groudtruth_num = 100000;
    const int N = maxNum*groudtruth_num;
    int *hist = (int *)malloc(N * sizeof(int));
    int *bin = (int *)malloc(maxNum * sizeof(int));
    int *bin_data;
    int *hist_data;
    cudaMalloc((void **)&bin_data, maxNum * sizeof(int));
    ...
    int *groudtruth = (int *)malloc(maxNum * sizeof(int));;
    for(int j = 0; j < maxNum; j++){
        groudtruth[j] = groudtruth_num;
    }
    ...
    cudaMemcpy(bin, bin_data, maxNum * sizeof(int), cudaMemcpyDeviceToHost);
    // bug2: 同bug1，L67传进去的256表示两个buffer的数据量，这个必须得精确，之前传的N，尽管只打印第1个值，但依然导致L27打印出来的值为垃圾值
    bool is_right = CheckResult(bin, groudtruth, maxNum);
    if(is_right) {
